...
public class ProfileUpdate extends JFrame {
    ...
    public void updateProfile() {
        String sql = "UPDATE user SET Fname=?, Lname=?, DoB=?, Role=?, Enrollment_Date=?, " +
                "Address=?, Email=?, Phone_No=?, Password=?, Profile_pic=? WHERE UserName=?";
        try (Connection conn = DatabaseCon.connect();
            ...
            try { ps.setInt(8, Integer.parseInt(textField9.getText())); }
            catch (NumberFormatException ex) { ps.setNull(8, java.sql.Types.INTEGER); }
            ps.setString(9, textField10.getText());
            ps.setString(10, textField11.getText());
            ps.setString(11, textField1.getText());

            int rows = ps.executeUpdate();
            if (rows > 0) JOptionPane.showMessageDialog(null, "Profile updated successfully.");
            else JOptionPane.showMessageDialog(null, "No profile found for that username.");
